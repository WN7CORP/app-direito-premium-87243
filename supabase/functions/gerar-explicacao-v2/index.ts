...
const REVISION = "v2.0.0-2025-11-05T02:30Z";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!DIREITO_PREMIUM_API_KEY) {
      ...
      return new Response(
        JSON.stringify({
          error: 'DIREITO_PREMIUM_API_KEY não configurada',
          provider: 'google',
          model: MODEL,
          status: 500,
...
- Mínimo de 2000 palavras no total`
    ...
    if (!response.ok) {
      ...
      console.error(`❌ Erro da API (modelo: ${MODEL}):`, response.status, errorText);
      
      // Retornar erro aprimorado com payload detalhado
      ...
      return new Response(
        JSON.stringify({
          error: `Erro da API de IA: ${response.status}`,
          provider: 'google',
          model: MODEL,
