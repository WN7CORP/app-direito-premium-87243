...
const CrimesDemocraticosView = () => {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ['crimes-democraticos-articles-v3'],
    queryFn: async () => {
          ...
          <div ref={contentRef} className="max-w-4xl mx-auto px-4 py-6 space-y-4 font-serif-content">
            {isLoading ? (
              ...
              displayedArticles.map((article, index) => {
                ...
                if (!hasNumber) {
                  return (
                    <div key={article.id} className="text-center mb-2 mt-3 font-serif-content">
                      <div 
                        className="text-xs leading-snug font-semibold text-white/70 whitespace-pre-line px-2 py-1" 
                ...
                return (
                  <div 
                    ...
                    className="relative bg-card rounded-2xl p-6 mb-6 border border-border/50 hover:border-[hsl(45,93%,58%)]/30 transition-all duration-300 animate-fade-in hover:shadow-lg scroll-mt-4"
                    style={{
