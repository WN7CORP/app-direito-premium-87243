      ...
      <div className="max-w-[600px] lg:max-w-4xl mx-auto px-4 py-6">
        <div className="relative space-y-6">
          ...
          {temasFiltrados.map((temaData, index) => {
            ...
            const prevArea = index > 0 ? temasFiltrados[index - 1].area : null;
            const showAreaHeader = activeTab === "todos" && temaData.area !== prevArea;

            return (
              <div key={`${temaData.area}-${index}`}>
                {/* Header da Área (apenas em "todos") */}
                {showAreaHeader && (
                  <div className="mb-4 pl-8">
                    <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${corArea}`} />
                      {temaData.area}
                    </h3>
                  </div>
                )}
                ...
                {/* Card do Tema */}
                <div className="relative pl-8 animate-fade-in-up" style={{
                  ...
                  >
                    ...
                    <div className="p-5 relative z-10">
                      {activeTab === "todos" && (
                        <div className="mb-2">
                          <span className={`${corArea} text-white px-2 py-1 rounded text-xs font-semibold`}>
                            {temaData.area}
                          </span>
                        </div>
                      )}
