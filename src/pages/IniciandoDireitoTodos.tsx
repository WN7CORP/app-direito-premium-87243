              ...
              dragFree: true
                    ...
                    <CarouselContent className="-ml-3 md:-ml-4">
                      {temasArea.map((temaData, index) => <CarouselItem key={`${area}-${index}`} className="pl-3 md:pl-4 basis-[70%] md:basis-[300px]">
                          <Card className="h-full cursor-pointer transition-all duration-300 overflow-hidden shadow-xl hover:shadow-2xl hover:scale-105 bg-card border-border/50 group" onClick={() => navigate(`/iniciando-direito/${encodeURIComponent(temaData.area)}/aula/${encodeURIComponent(temaData.tema)}`)}>
                            {/* Imagem de capa */}
