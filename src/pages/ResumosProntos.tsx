          ...
          <div className="relative max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 mb-10 px-4">
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl md:text-4xl font-bold text-white">{areas?.length || 0}</span>
                <span className="text-sm text-white/80">Áreas</span>
              </div>
              <div className="w-px h-10 bg-white/30" />
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl md:text-4xl font-bold text-white">{totalResumos}+</span>
                <span className="text-sm text-white/80">Resumos</span>
              </div>
              <div className="w-px h-10 bg-white/30" />
              <div className="flex items-baseline gap-1.5 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2">
                <span className="text-2xl md:text-3xl font-bold text-white">2026</span>
                <span className="text-sm text-white/80">Atualizado</span>
              </div>
            </div>
            ...
            {loadingAreas ? (
              ...
              <div className="max-w-xl md:max-w-3xl mx-auto space-y-4">
                {areasFiltradas.map((area, idx) => (
                  ...
                  >
                    <Card
                      className="group cursor-pointer hover:shadow-2xl hover:shadow-purple-500/20 transition-all duration-300 hover:scale-[1.01] border border-border/80 bg-gradient-to-br from-card to-card/80 min-h-[116px] md:min-h-[124px] flex"
                      onClick={() => {
                        setAreaSelecionada(area.area);
                        window.scrollTo({ top: 0, behavior: 'smooth' });
                      }}
                    >
                      <CardContent className="flex-1 flex items-center gap-4 p-5 md:p-6">
                        {/* Ícone do livro */}
                        <div className="flex-shrink-0">
                          <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center shadow-lg transform group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <h3 className="font-bold text-base md:text-lg mb-1.5 group-hover:text-purple-400 transition-colors line-clamp-2">
                              {area.area}
                            </h3>
                            <span className="ml-2 px-2.5 py-1 rounded-full text-xs md:text-sm font-semibold bg-purple-500/15 text-purple-300 whitespace-nowrap">
                              {area.count}
                            </span>
                          </div>
          ...
          {loadingTemas ? (
            ...
            <div className="space-y-4">
              {temasFiltrados?.map((tema, idx) => (
                ...
                >
                  <Card
                    className="group cursor-pointer hover:shadow-xl hover:shadow-purple-500/10 transition-all duration-300 hover:scale-[1.01] border border-border/80 bg-card/95 min-h-[120px] md:min-h-[130px]"
                    onClick={() =>
                      navigate(
                    ...
                    <CardContent className="p-5 md:p-6">
                      <div className="flex items-start justify-between gap-3 md:gap-4">
                        <div className="flex items-start gap-3 md:gap-4 flex-1 min-w-0">
                          {/* Número do tema */}
                          <div className="flex-shrink-0 w-11 h-11 md:w-12 md:h-12 bg-gradient-to-br from-purple-500 to-blue-500 rounded-xl flex items-center justify-center text-white font-bold text-base md:text-lg shadow-lg group-hover:scale-110 transition-transform">
                            {idx + 1}
                          </div>
                          
                          {/* Conteúdo */}
                          <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-sm md:text-base lg:text-lg mb-1.5 md:mb-2 group-hover:text-purple-400 transition-colors line-clamp-2">
                              {tema.tema}
                            </h3>
                            <div className="flex items-center flex-wrap gap-2 md:gap-3 text-xs md:text-sm text-muted-foreground">
                        ...
                        <div className="flex-shrink-0 w-9 h-9 md:w-10 md:h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary/20 transition-colors">
