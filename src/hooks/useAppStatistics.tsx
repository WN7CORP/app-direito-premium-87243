  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<AppStatistics> => {
      ...
      const [
        ...
        supabase.from<any>("FLASHCARDS").select("*", { count: "exact", head: true }),
        supabase.from<any>("VIDEO AULAS-NOVO").select("*", { count: "exact", head: true }),
        supabase.from<any>("AUDIO-AULA").select("*", { count: "exact", head: true }),
        supabase.from<any>("BIBLIOTECA-ESTUDOS").select("*", { count: "exact", head: true }),
        supabase.from<any>("BIBLIOTECA-FORA-DA-TOGA").select("*", { count: "exact", head: true }),
        supabase.from<any>("BIBLIOTECA-CLASSICOS").select("*", { count: "exact", head: true }),
        supabase.from<any>("BIBLIOTECA-LIDERANÇA").select("*", { count: "exact", head: true }),
        supabase.from<any>("BIBLIOTECA-ORATORIA").select("*", { count: "exact", head: true }),
        supabase.from<any>("RESUMO").select("*", { count: "exact", head: true }),
        supabase.from<any>("SIMULADO-OAB").select("*", { count: "exact", head: true }),
        supabase.from<any>("CURSOS").select("*", { count: "exact", head: true }),
        supabase.from<any>("SIMULACAO_CASOS").select("*", { count: "exact", head: true }),
        supabase.from<any>("MAPA MENTAL" as any).select("*", { count: "exact", head: true }),
      ]);
